<x-layouts.app title="Edit Hearing" header="Edit Hearing">
    <!-- Breadcrumb -->
    <nav class="mb-6">
    ...
    <form action="{{ route('hearings.update', $hearing) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="case_id" id="case_id" required
                        ...
                        <option value="{{ $case->id }}" {{ old('case_id', $hearing->case_id) == $case->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="date_time" id="date_time" required
                        value="{{ old('date_time', $hearing->date_time->format('Y-m-d\TH:i')) }}"
                ...
                <div>
                    ...
                    <input type="text" name="court_branch" id="court_branch"
                        value="{{ old('court_branch', $hearing->court_branch) }}"
                ...
                <div>
                    ...
                    <select name="assigned_prosecutor_id" id="assigned_prosecutor_id"
                        ...
                        <option value="{{ $prosecutor->id }}" {{ old('assigned_prosecutor_id', $hearing->assigned_prosecutor_id) == $prosecutor->id ? 'selected' : '' }}>
                            {{ $prosecutor->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="result_status" id="result_status"
                        ...
                        <option value="Scheduled" {{ old('result_status', $hearing->result_status) == 'Scheduled' ? 'selected' : '' }}>Scheduled</option>
                        <option value="Completed" {{ old('result_status', $hearing->result_status) == 'Completed' ? 'selected' : '' }}>Completed</option>
                        <option value="Postponed" {{ old('result_status', $hearing->result_status) == 'Postponed' ? 'selected' : '' }}>Postponed</option>
                        <option value="Cancelled" {{ old('result_status', $hearing->result_status) == 'Cancelled' ? 'selected' : '' }}>Cancelled</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="remarks" id="remarks" rows="3"
                        ...
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('remarks') border-red-500 @enderror">{{ old('remarks', $hearing->remarks) }}</textarea>
                    @error('remarks')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="flex items-center justify-end space-x-4">
            ...
            <button type="submit" class="px-6 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors">
                <i class="fas fa-save mr-2"></i>Update Hearing
            </button>
        </div>
    </form>
</x-layouts.app>
